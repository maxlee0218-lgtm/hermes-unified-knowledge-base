...
当前采用"首脑判断、Multica 分派、Windows Runtime 执行、DataGrip/MCP 统一数据访问、llm-wiki 沉淀"的协作结构。

> ⚠️ **当前状态**：Windows 已重置，Multica 及所有智能体已废弃，本架构处于**冻结状态**。
...
| Multica | 任务平台 | Issue、Agent 分派、状态跟踪 | 不替代 llm-wiki |
| 任务管家-Windows-Kimi | 任务中转 | 接收、拆分、分派、汇总 | ❌ 已废弃（Windows重置） |
| 数据虾-Windows-Kimi | 数据分析 | 只读分析、报告输出 | ❌ 已废弃（Windows重置） |
| 代码工-Windows-Codex | 代码能力 | 脚本、SQL、代码生成和检查 | ❌ 已废弃（Windows重置） |
